...
public class BossEnemyController : EnemyControler
{
    ...
    void FixedUpdate()
    {
        switch (this.state)
        {
            case BossEnemyState.IDLE:
                break;
            case BossEnemyState.CHASING:
                direction = player.transform.position - this.gameObject.transform.position;
                Rotate();
                FollowPath();
                break;
            case BossEnemyState.ATTACKING:
                direction = player.transform.position - this.gameObject.transform.position;
                Rotate();
                break;
            case BossEnemyState.CHARGING_UP:
                direction = player.transform.position - this.gameObject.transform.position;
                Rotate();
                break;
            case BossEnemyState.CHARGING:
                Vector2 velocity = direction * movementSpeed * 2f;
                velocity = velocity * Time.deltaTime;

                transform.position = new Vector3(transform.position.x+velocity.x, transform.position.y+velocity.y, -0.4f);
    ...
    */
    ...
    public void StartChargingUp()
    {
        this.state = BossEnemyState.CHARGING_UP;
    }

    public void StopChargingUp()
    {
    ...
    {
        IDLE, CHASING, ATTACKING,CHARGING_UP,CHARGING
    }
